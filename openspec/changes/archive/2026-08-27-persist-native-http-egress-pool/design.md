...
- Closing a response unregisters its queue and sends `cancel`; it does not terminate the helper. Closing the client closes stdin, terminates if needed, awaits the process and reader, and fails remaining requests.
...
- [A stalled consumer can block demultiplexing] -> Use bounded queues, require response close in every terminal path, and test cancellation under concurrent streams.
